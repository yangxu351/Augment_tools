...
def alb_aug_cls(base_dir, split=['val'], method_params={}, combine=False, exec_num=1):
    ...
    for sp in split:
        source_data_dir = os.path.join(base_dir, 'inputs', f'{sp}_dataset')
        # source_dir = os.path.join(base_dir, split)
        save_dir = os.path.join(base_dir ,  'outputs', f'Alb_aug_{sp}_dataset')
        make_dir_if_not_exist(save_dir, rm=False)

        cat_folders = os.listdir(source_data_dir)
    
        for cf in cat_folders:
            img_names = os.listdir(os.path.join(source_data_dir, cf))
            img_names.sort()
            img_list = []
            for en in range(exec_num):
                ...
                for key, aug in dict_trans.items():
                    ...
                    for n in img_names:# FIXME:
                        img = np.array(Image.open(os.path.join(source_data_dir,cf,n)))
